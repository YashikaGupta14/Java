...
public class ComplaintRegisterController {
	...
	public ModelAndView loadComplaintForm() {
		...
		andView.setViewName("ComplaintPage");
		andView.addObject("complaintBean",new ComplaintBean());
...
//		return new ModelAndView("ComplaintPage", "complaintBean", new ComplaintBean());
	...
	public ModelAndView processComplaintForm(@ModelAttribute("complaintBean") @Valid ComplaintBean complaintBean,BindingResult result) throws Exception {
		...
		if(result.hasErrors()) {
			...
			try {
		int a= complaintService.registerComplaintDetails(complaintBean);
		andView.setViewName("Success" );
			andView.addObject("message", complaintBean.getCustomerName()+" your complaint is successfully registered." );
	...
	public ModelAndView handleAllExceptions(Exception exception) {
		 // Your implementation goes here
		ModelAndView andView= new ModelAndView();
		andView.setViewName("GeneralizedExceptionHandlerPage");
		andView.addObject("message", exception.getMessage());
		andView.addObject(exception);
        return andView;
	}
}
