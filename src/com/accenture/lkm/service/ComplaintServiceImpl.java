...
public class ComplaintServiceImpl implements ComplaintService {
	...
	public int registerComplaintDetails(ComplaintBean complaintBean) throws Exception {
		...
		if(count==1) {
			throw new Exception ("Customer has Already Submitted");
		}
	...
	public List<ComplaintBean> getComplaintDetailsByDate(Date fromDate, Date toDate) {
		// Your implementation goes here
	List<ComplaintBean> listComplaintBean=	complaintDaoWrapper.getComplaintDetailsByDate(fromDate, toDate);
	
	return listComplaintBean;
	...
	public Map<Integer, String> getAllComplaintTypes() {
		...
		Map<Integer, String>mapComplaintType= new HashMap<>();
	
		 List<ComplaintTypeBean> list=complaintDaoWrapper.getAllComplaintTypes();
		 for(ComplaintTypeBean item : list) {
			 mapComplaintType.put(item.getComplaintTypeId(), item.getComplaintTypeName());
		 }
